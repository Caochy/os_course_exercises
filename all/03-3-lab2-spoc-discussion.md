...
2. 不同特权级有什么区别？
    ...
    - 特权指令只有在特权模式下才能使用
    - 不同特权级下指令的数据访问范围是不同的
    - 对于同一条指令，在不同特权级下的表现也是不同的

3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。

    - 对于数据段，DPL规定了数据段中的最低的访问权限。
    - 只有在特定的特权级下才能执行一些特权指令。
...
4. 如何利用int和iret指令完成不同特权级的切换？
    ...
    通过人工构造栈结构，对需要存储的数据进行压栈处理，从而可以通过int和iret指令进行特权级的切换。
